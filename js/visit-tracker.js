...
async function _vtGetIP() {
  ...
  for (const url of sources) {
    try {
      ...
      return { ip: d.ip || d.query || '未知', country: d.country || d.country_name || '未知', region: d.region || d.regionName || '', city: d.city || '' };
    } catch (e) { continue; }
  }
...
async function visitTracker() {
  ...
  try {
    ...
    await fetch(`https://api.github.com/repos/${cfg.user}/${cfg.visitRepo}/issues/${issue.number}/comments`, {
      method: 'POST',
      headers: { 'Authorization': 'token ' + cfg.token, 'Content-Type': 'application/json' },
      body: JSON.stringify({ body })
    });
  } catch (e) { console.warn('visitTracker:', e.message); }
}
